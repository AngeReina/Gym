...
public class SuscripcionService {
    ...
    public void crearSuscripcion(SuscripcionDTO dto) {
        ...
        int dias = plan.getDuracionDias() != null
        ...
        s.setCosto(plan.getCosto().doubleValue());
    ...
    public List<SuscripcionDTOResponse> porCliente(Long idCliente) {

        return suscripcionRepository.porCliente(idCliente)
                .stream()
                .map(s -> new SuscripcionDTOResponse(
                        ((Number) s[0]).longValue(),
                        (String) s[4] + " " + (String) s[5],
    ...
    public List<SuscripcionDTOResponse> porEstado(String estado) {

        return suscripcionRepository.porEstado(estado)
                .stream()
                .map(s -> new SuscripcionDTOResponse(
                        ((Number) s[0]).longValue(),
                        (String) s[1],
